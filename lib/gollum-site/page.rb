module Gollum
  class SitePage < Gollum::Page
    ...
    def generate(output_path, version)
      ...
      #::Dir.mkdir(::File.dirname(::File.join(output_path, self.class.cname(name))))
      ::Dir.mkdir(::File.join(output_path, self.class.cname(name))) unless ::File.exists? ::File.join(output_path, self.class.cname(name))
      ::File.open(::File.join(output_path, self.class.cname(name), '/index.html'), 'w') do |f|
        f.write(data)
      end
    end
